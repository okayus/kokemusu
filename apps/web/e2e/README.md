...
- `golden-path` — WebAuthn 仮想 authenticator（CDP）で初回登録 → リロードでセッションが残る → 苔片を 1 つ積む → タイムラインに復号された本文とタグ → 総草の今日のマスが level 0 → 1（画面と API の両方）→ リロードで残る → ログアウト → 登録したパスキーでログイン → 2 本目の苔片でタグ絞り込み 3 導線と Markdown 描画 → 編集・削除 → 期間絞り込み（今日プリセット → 苔片を at rest で 1 日戻して JST 境界を wire で検査 → カスタム範囲 → 逆転はブラウザの検証で止まりリクエストにならない）。`DEV_BYPASS_USER_ID` は作らない（配線を素通りしたら回帰を拾えない）
