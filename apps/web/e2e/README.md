...
- `golden-path` — WebAuthn 仮想 authenticator（CDP）で初回登録 → リロードでセッションが残る → 苔片を 1 つ積む → タイムラインに復号された本文とタグ → 総草の今日のマスが level 0 → 1（画面と API の両方）→ リロードで残る → ログアウト → 登録したパスキーでログイン → 2 本目の苔片でタグ絞り込み 3 導線と Markdown 描画 → 編集（編集中は右下の丸い「積む」が消えて同じ位置にその編集フォームの「保存」、ほかの苔片の「編集」は押せない、右下の「保存」で保存すると「積む」が戻る）・削除 → 期間絞り込み（今日プリセット → 編集フォームの「日: YYYY/MM/DD」を開いて 2 欄とも昨日にして保存 ＝ PATCH で苔片の「日」（`first_day` / `last_day`）を 1 日戻し、今日の窓から消え重なりを wire で検査、カードは日と「M/D に積む」で時刻無し → カスタム範囲 → 逆転はブラウザの検証で止まりリクエストにならない）→ 総草のマスのタップ（リロード後、Tab 停止は今日のマス 1 つ → ↑ で前日のマスへ移り停止も移る → Enter で投稿一覧がその日に絞られチップ「YYYY/MM/DD ×」→ 今日のマスのクリックは空 → × で戻る）→ 積むダイアログ（右下の丸い「積む」で開く。1 本目はここから、2 本目は `n` キーで開く。前の投稿のタグは持ち越されず、Esc で閉じた本文は次に開くと戻る → 昨日に絞った状態で積むと一覧は動かずバーに「積みました（いまの絞り込みの外）」、苔は濃くなる → 苔片の「同じ石に積む」でタグ欄がその石のチップで埋まり向きもその苔片のもの（未分類 —— 直前に下書きへ残したアウトプットを置き換える）、本文は空、先頭に着地）→ タグ欄は combobox（2 本目で: 「e」で候補 e2e → Enter でチップ、未登録の綴りは「…を新しい石に」、Escape は候補だけ畳んでダイアログは残る、空欄の Backspace でチップを外す。編集フォームは苔片の石がチップで入った状態から 1 つ足す）→ 向き（インプット 2 枚（石「読書」）で今日のマスの読み上げ・比率・wire・カードの印、編集で未分類へ戻すと追随 → 石のつながりの届く範囲とスポットライト（2026-09-14。石「読書」は「苔」と橋が無い: 「苔」をホバーすると「読書」が薄れ（`data-far`）「e2e」は薄れず橋 e2e–苔 が濃くなる（`data-near`）、ポインタを離すと戻る → 「苔」をクリックすると「読書」が隠れ（`data-hidden` を `visibility` で消すので role でも掴めない）「e2e」は残り絞り込みチップ「苔」が立つ、もう一度で戻り一覧は 5 片） → 残ったインプットの「同じ石に積む」でチップ「読書」とインプットが入る、未分類に戻しチップを外して Esc で閉じれば下書きに残らない）→ 過去に積む・続く苔片（「日を選ぶ」で昨日に 1 枚 → 一覧は動かず受領に「YYYY/MM/DD に絞る」ボタン、押すとその日に絞られカードは日 ＋「M/D に積む」、昨日のマス +1・計 +1 → いつ ＝ 昨日・〜いつまで ＝ 今日の続く苔片 → 両日 +1 だが計は +1、年表の行は昨日〜今日で触れる月に 1、今日のマスのクリックで出る（重なり）→ 逆転は「いつ」の rangeOverflow で止まりダイアログは残る → in-page fetch で 未来・非日付・逆転・1900 年・PATCH で明日へ伸ばす が全部 400 で行は不変 → 厚み（ADR-0007）: いつ だけでは「厚み」のスライダーは無く、〜いつまで を入れると現れて 100 と換算「100% · 2 日のうち 2 日分」、そのまま積めば wire と at rest に 100 でカードは「… · 厚み 100%」、単日は null。単日に厚み・範囲に厚み無し・PATCH で単日に縮めて厚みが残る・範囲の厚みだけ外す も 400 で行は不変 → 編集フォームの summary「日: … · 厚み 100%」を開くとスライダーは苔片の 100、60 にすると換算「60% · 2 日のうち 1 日分」（量の丸めと同じ）、保存でカード「厚み 60%」・行に 60・年表の行の量 1.2 → 量（ADR-0007 の集計）: in-page fetch で 10 日 × 厚み 60 の続く苔片を積むと wire は `amount` 6 / `count` 1 / `months` の和 6、`?period=month` は今月に重なる日数 × 0.6、リロード後に石の名前「案件 · 量 6」と、年表の棒をホバー（Chromium は interest invoker）／クリック（popovertarget）で開く吹き出しの「量 6 · 厚み 60%」（当たり判定 button の aria-label は「… · 1 片 · … · 量 6 · 厚み 60%」、Escape で閉じる）→ 本文はカードから溢れない（2026-09-13）: 幅 390px で長い 1 行のコードブロックの苔片を in-page fetch で積んでも `.post-body` はカード幅のまま、`pre` は枠内で横スクロール、ページの scrollWidth は viewport と同じ（広がると iPhone の WebKit がその苔片の文字だけ拡大していた）。石・橋の名前は 2026-09-12 から「量 N」、年表の数値は 2026-09-13 から吹き出し（行の右の注記は無い）。同日から行の石チップの下に時代の文字「YYYY/MM 〜 YYYY/MM」（同じ月なら 1 つ）、軸の左端は始まりの「YYYY年M月」・右端は「今日」で、吹き出しと当たり判定の日はスラッシュ綴り。`DEV_BYPASS_USER_ID` は作らない（配線を素通りしたら回帰を拾えない）
- `auth-boundary` — セッション無しの `/api/*` は読み書き・未知パスとも 401 JSON（SPA に落ちない）。偽 Cookie は 401、登録トークン違いは 403、`login/begin` は公開のまま（mount 順の回帰検知）
- `pat` — 設定 UI で発行（一度きり表示）→ Cookie 無しの送り側が `Authorization: Bearer` だけで `POST /api/posts`（Origin 不要 = CSRF 免除。body は `kind: "input"` と `firstDay: 昨日` — mazuoboeru の朝の push の形で、応答は `firstDay` / `lastDay` ＝ 昨日・`postedDay` ＝ 今日。その前に `title` を持つ body は 400 — ADR-0006 で消えた見出しを送る送り側は黙って削られず断られる。厚み（ADR-0007）: 単日 ＋ `thickness` は 400、範囲だけ・範囲 ＋ null は 400、範囲 ＋ 60 は 201 で応答と D1 に 60、単日の応答は null）→ 積まれた苔片は暗号化されて UI に復号表示（昨日の位置、カードは日）→ post:write はタイムラインを読めない・PAT で PAT は作れない（403 `session_required`）・PAT は同乗 Cookie より先に判定 → UI で失効すると 401。D1 に生トークンが無いこと、`first_day` / `last_day` / `kind` が平文で入っていることも見る
