...
- `docs/dev-environment.md` — 開発サンドボックス・GitHub token 注入・Workers Builds キーレスデプロイ・デプロイ骨格（2026-08-22 に現行化、PR #3）
...
- **フロント = React 19 + Vite + TS / API = Hono / DB = D1 + Drizzle**。toolchain は wrangler 4 + `@cloudflare/vite-plugin` 1.x（deploy-skeleton の現行基準）。**node は host / sandbox / CI / Workers Builds とも 24**。
- **配置（2026-08-22 骨格）**: pnpm workspace。**`apps/web`** に SPA（`src/`）＋ Worker（`worker/{index,cron,types}.ts`）＋ `drizzle/`（D1 マイグレ）＋ `wrangler.jsonc`（`name: kokemusu`、3 層 SPA、D1 `kokemusu-db`、Cron `15 * * * *`、`vars.RP_ID / ORIGIN` は本番で固定）。`packages/core` はロジックが生えたら。骨格の版: wrangler 4.125 / vite-plugin 1.53 / Vite 8.2 / React 19.2 / Hono 4.13 / TypeScript 7.0 / Vitest 4.1。型は `wrangler types`（`worker-configuration.d.ts` は gitignore。`pnpm check` = `wrangler types && tsc --noEmit`）。単体テストは `vitest.config.ts`（Cloudflare plugin を通さず Node で `app.request()`）。ローカルは `apps/web/.dev.vars`（`.dev.vars.example` をコピー）で `RP_ID=localhost`。
...
4. ~~docs の drift 修正~~ → **PR #3（2026-08-22）**。~~`/grill-with-docs`~~ → **完了（2026-08-23、PR #5 = #3 の上に stacked）**: 決めること 2 = 本文はアプリ層で暗号化 (B)・鍵は Worker Secret・(C) 不採用（ADR-0001）／7 = 見出しは任意・手動でも API でも・UI は既定で隠す／8 = mazuoboeru 連携は (i) 自分専用、受け側 PAT は送り側を知らない（ADR-0002）。用語集 `CONTEXT.md` を新設
5. Phase 0: ~~deploy-skeleton~~ → **骨格 PR（`claude/phase0-skeleton`）**。ローカルで `pnpm check` / `test` / `build` / `db:migrate` / `pnpm dev` の `/health`・SPA fallback・`/cdn-cgi/handler/scheduled` を確認済み。**残りは人手**: (a) ホストで `wrangler login` → `wrangler d1 create kokemusu-db` → `apps/web/wrangler.jsonc` の `database_id` placeholder を実 UUID に置換（UUID は秘密ではない。エージェントに渡して置換させてもよい） (b) 骨格ブランチの最後の commit（`.github/workflows/ci.yml` を typecheck / build / test に置換）を **ホストから `git push origin claude/phase0-skeleton`**（token に workflows 権限が無い） (c) dash で Workers Builds 接続: Worker 名 `kokemusu`・Root directory **`apps/web`**（Advanced settings）・Build `pnpm install --frozen-lockfile && pnpm run build`・Deploy `pnpm exec wrangler d1 migrations apply kokemusu-db --remote && pnpm exec wrangler deploy`・**D1 Edit 入りカスタムビルドトークン**・**非本番ブランチビルド OFF**・watch paths で `docs/*` `*.md` を除外 (d) PR merge → `curl https://kokemusu.shiraoka.workers.dev/health` → `{"status":"ok"}`、`/` が SPA HTML、ホストで `wrangler d1 migrations list kokemusu-db --remote` に `0000_init`
