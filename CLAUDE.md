...
3. E2E（token 経路: push → `gh pr create` → `gh pr checks`、PR の無い commit での `main` push 拒否、workflows 変更の拒否、bypass 下の deny、`down`→plain `up` の fail closed）＋ docs 参照 3 層の疎通（context7 で MDN、`hono.dev/llms.txt` の WebFetch）
