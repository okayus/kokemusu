...
| `cloudflare-d1-keyless-host-backup` | D1 の定期バックアップ（ホストの週次 timer）・migration 前の Time Travel bookmark・復元 | **稼働（2026-09-20）**。public リポなので `cloudflare-d1-weekly-backup-via-pr`（git に commit）は使わない |
