...
- **(B) アプリ層の本文暗号化（← 採用）**: Worker が保存時に本文を AES-GCM で暗号化し、読み出し時に復号。鍵 `BODY_KEY` は Worker Secret。
...
- → **タグ名・タグ説明も平文側**（2026-09-02。案件をタグで表す帰結）: 案件タグは顧客の固有名詞を避けた一般化名にする（「病院予約」等）。実名・固有情報は暗号化される本文に書く。
