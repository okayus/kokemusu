...
- **厚み**（2026-09-09 決定、[ADR-0007](adr/0007-spanning-post-amount-is-days-times-thickness.md)。続く苔片の量 ＝ 日数 × 厚み。
  PR 1 再構築 ＋ core ＋ API（人の merge）→ PR 2 集計 → PR 3 UI、[plans/thickness.md](plans/thickness.md)。エクスポートより先）。
...
12. ~~**続く苔片の量（期間で積むと 1 回分にしかならない）**~~ → ✅ **続く苔片は「厚み」（期間のうち打ち込んでいた日の割合、1〜100%）を
    必ず持ち、量 ＝ 日数 × 厚み。単日は 1 で厚みを持たない。「単日 | 続く(厚み)」の直和を core の型・DB の CHECK（`post` の 2 度目の
